...
* @param  x: 起点 X 轴坐标（像素）
* @param  y: 起点 Y 轴坐标（像素）
* @param  ch: 需要显示的ASCII字符
* @param  reverse_flag: 反色显示标志位，0-正常，1-反色
void (*Display_Char)(unsigned short int x, unsigned short int y, char ch, unsigned char reverse_flag);

* @brief  ASCII字符显示函数（Y 轴以行为单位）
* @param  x: 起点 X 轴坐标（像素）
* @param  line: 对应行（Y坐标 = line * CHAR_HEIGHT）
* @param  ch: 需要显示的ASCII字符
* @param  reverse_flag: 反色显示标志位，0-正常，1-反色
void (*Display_Char_Line)(unsigned short int x, unsigned char line, char ch, unsigned char reverse_flag);

* @brief  UTF-8字符显示函数（支持中文等多字节字符）
* @param  x: 起点 X 轴坐标（像素）
* @param  y: 起点 Y 轴坐标（像素）
* @param  str: UTF-8字符字符串指针
* @param  len: UTF-8字符长度（字节数：2/3/4）
* @param  reverse_flag: 反色显示标志位，0-正常，1-反色
void (*Display_UTF8_Char)(unsigned short int x, unsigned short int y, char* str, unsigned char len, unsigned char reverse_flag);

* @brief  UTF-8字符显示函数（Y 轴以行为单位）
* @param  x: 起点 X 轴坐标（像素）
* @param  line: 对应行（Y坐标 = line * CHAR_HEIGHT）
* @param  str: UTF-8字符字符串指针
* @param  len: UTF-8字符长度（字节数：2/3/4）
* @param  reverse_flag: 反色显示标志位，0-正常，1-反色
void (*Display_UTF8_Char_Line)(unsigned short int x, unsigned char line, char* str, unsigned char len, unsigned char reverse_flag);
...
typedef struct Easy_Menu {
    ...
    void (*Display_Char_Line)(unsigned short int x, unsigned char line, char ch, unsigned char reverse_flag);               // 核心显示函数（ASCII，按行）
    
    void (*Display_UTF8_Char)(unsigned short int x, unsigned short int y, char* str, unsigned char len, unsigned char reverse_flag);         // 核心显示函数（UTF-8字符）
    void (*Display_UTF8_Char_Line)(unsigned short int x, unsigned char line, char* str, unsigned char len, unsigned char reverse_flag);      // 核心显示函数（UTF-8字符，按行）
...
/**
    * @brief  菜单系统初始化
    * @param  Display_Char: ASCII字符显示函数（像素坐标）
    * @param  Display_Char_Line: ASCII字符显示函数（行坐标）
    * @param  Display_UTF8_Char: UTF-8字符显示函数（像素坐标），传NULL则不支持UTF-8字符
    * @param  Display_UTF8_Char_Line: UTF-8字符显示函数（行坐标），传NULL则不支持UTF-8字符
    * @notes  Display_Char和Display_Char_Line至少绑定一个系统才能显示
  */
void Easy_Menu_Init(void (*Display_Char)(unsigned short int x, unsigned short int y, char ch, unsigned char reverse_flag), 
                    void (*Display_Char_Line)(unsigned short int x, unsigned char line, char ch, unsigned char reverse_flag),
                    void (*Display_UTF8_Char)(unsigned short int x, unsigned short int y, char* str, unsigned char len, unsigned char reverse_flag), 
                    void (*Display_UTF8_Char_Line)(unsigned short int x, unsigned char line, char* str, unsigned char len, unsigned char reverse_flag));
